...
# Find earliest and latest end of support dates
eol_dates = []
for device in devices:
    eol_date = device.get('end_of_support_date', '')
    if eol_date and eol_date not in ['Current', 'Discontinued BUT supported', '(current)', '4 years from release date']:
        eol_dates.append((device['device_name'], eol_date))

# Sort by date if possible
sorted_eol_dates = []
for name, date_str in eol_dates:
    ...
    try:
        ...
        if re.match(r'\d{4}-\d{2}-\d{2}', date_str):
            ...
            date_obj = datetime.strptime(date_str, '%b %Y')
        sorted_eol_dates.append((name, date_str, date_obj))
    except:
        # If parsing fails, just keep the string
        sorted_eol_dates.append((name, date_str, None))

# Sort by date
sorted_eol_dates = [x for x in sorted_eol_dates if x[2] is not None]
if sorted_eol_dates:
    sorted_eol_dates.sort(key=lambda x: x[2])
...
if sorted_eol_dates:
    earliest = sorted_eol_dates[0]
    latest = sorted_eol_dates[-1]
...
discontinued_but_supported = sum(1 for device in devices if device.get('end_of_support_date') == 'Discontinued BUT supported')
